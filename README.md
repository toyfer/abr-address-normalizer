...
複数の住所を1行に1件ずつ貼り付けて一括正規化できます。

- **入力**: 1行1住所。空行は無視、最大 **500件** まで
- **表示**: 行番号・入力住所・正規化済み住所・レベル・判別不能部分・成否・緯度経度 の表
- **集計**: 成功/失敗件数の表示
- **進捗**: プログレスバーと件数表示
- **エクスポート**:
  - **CSVダウンロード** — Excelでそのまま開けるようUTF-8 BOM付き（列: 入力住所/都道府県/市区町村/町字/街区符号・住居符号／地番/正規化済み住所/正規化レベル/緯度/経度/判別不能部分/エラー）
  - **TSVコピー** — Excel・スプレッドシートにそのまま貼り付け可能
  - **JSONコピー** — 他システムへの取り込み用

> 処理はサーバーに負荷をかけないよう1件ずつ順次実行します。初回は住所データのダウンロード（県ごとのJSON）があるため数秒かかることがあります。

### 表示テーマ

ライト / ダーク / 自動（OS設定に追従）を右上で切り替え可能。選択はブラウザに保存されます。
